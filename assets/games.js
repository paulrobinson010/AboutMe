...
/* A fourth taster, for NoobCube: a real 3x3 you can turn.

   Each cubie carries its own orientation matrix. A layer turn multiplies
   that matrix and re-lays the cubie out with the rotation applied before
   the translation, so it orbits the middle of the cube rather than
   spinning where it stands. Solve it winds the whole history back. */
  /* ---------- 7 — the Cubebox: a cube you can actually turn ---------- */
  (() => {
    ...
    document.querySelectorAll("#open-cubebox, .t-noob .mark").forEach((el) => {
